...
const withWidgetSourceCodes = (config, { widgetName, appGroupName }) => {
    return (0, config_plugins_1.withDangerousMod)(config, [
        ...
        async (newConfig) => {
            ...
            prepareSourceCodes(widgetDir, platformRoot, packageName, widgetName, appGroupName);
            return newConfig;
        },
    ]);
};
...
async function prepareSourceCodes(widgetSourceDir, platformRoot, packageName, widgetClassName, appGroupName) {
    ...
    const widgetSourceFilePath = path_1.default.join(source, `${widgetClassName}.kt`);
    if (!fs_1.default.existsSync(source)) {
        const templateFolder = path_1.default.join(__dirname, "static", "java/package_name");
        await fs_1.default.promises.cp(templateFolder, source, { recursive: true });
        await fs_1.default.promises.rename(path_1.default.join(source, "SampleWidget.kt"), widgetSourceFilePath);
        ...
        newContent = newContent.replace(/group.com.example.widget/, `${appGroupName}`);
